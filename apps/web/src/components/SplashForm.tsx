/**
 * SplashForm Component - Multi-Step Lead Capture Form
 * 
 * A sophisticated multi-step form with sliding animations for the Ameren Illinois
 * bill pay promotion. Features early TCPA consent for abandoned lead follow-up.
 * 
 * Features:
 * - 13 sequential form steps with slide animations
 * - TCPA consent at step 4 (after phone number) for abandoned lead compliance
...
const FORM_STEPS: FormStep[] = [
  ...
  // Step 4 (index 3) will be TCPA consent - handled separately
  {
    id: 'email',
  ...
  },
  {
    ...
    validation: (value) => !value.trim() ? 'Street address is required' : null
  },
  {
    id: 'city',
  ...
  },
  {
    ...
    validation: (value) => !value.trim() ? 'Utility company is required' : null
  },
  {
    id: 'homeownerStatus',
    ...
    validation: (value) => !value ? 'Please select your homeowner status' : null
  },
  {
    id: 'creditScore',
    ...
    validation: (value) => !value ? 'Please select your credit score range' : null
  },
  {
    id: 'shading',
    ...
    validation: (value) => !value ? 'Please select your home\'s shading condition' : null
  }
];
...
export function SplashForm() {
  ...
  useEffect(() => {
    localStorage.removeItem('quantumSolarSplashForm');
  ...
  const saveToLocalStorage = useCallback(() => {
    if (!sessionId) return;
    try {
      localStorage.setItem('quantumSolarSplashForm', JSON.stringify({
        ...formData,
        sessionId,
  ...
  useEffect(() => {
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      ...
      if (formData.tcpaConsent && formData.phone && currentStep > 3) {
  ...
  const checkDisqualification = (step: FormStep, value: string) => {
  ...
  const getCurrentStepInfo = () => {
    if (currentStep === 3) {
      ...
      // Steps 1-3 (firstName, lastName, phone)
      return { isConsentStep: false, step: FORM_STEPS[currentStep] };
    } else {
      // Steps after consent (email onwards) - adjust index
  ...
  const handleNext = async () => {
    ...
    if (!isConsentStep && step) {
      const currentValue = formData[step.field] as string;
    ...
    setTimeout(() => {
      if (currentStep < 12) { // Total of 13 steps (0-12)
    ...
    if (currentStep > 3 && currentStep % 3 === 0) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      trackLeadEvent('Ameren Illinois Splash Form');
      
      // Track GTM events
      trackFormSubmission('ameren_illinois_splash', {
        homeowner_status: formData.homeownerStatus,
        credit_score: formData.creditScore,
        utility_company: formData.utilityCompany
      });
      trackLead('ameren_illinois_splash');
      trackConversion('ameren_illinois', 'form_completion');
  ...
  const totalSteps = 13; // Total steps including consent step (no appointment fields)
          ...
          <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-8">
            ...
            >
              {isSubmitting ? 'Processing...' : 
               currentStep === 12 ? 'Submit Application' : 
