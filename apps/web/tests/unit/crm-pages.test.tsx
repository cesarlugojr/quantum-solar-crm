...
describe('CRM Pages', () => {
  ...
  describe('LeadsPage', () => {
    ...
    it('calls API to fetch leads on mount', async () => {
      ...
      await waitFor(() => {
        expect(apiCallSpy).toHaveBeenCalledWith('http://localhost:3000/api/crm/leads')
      })
    })
  })
  ...
  describe('ProjectsPage', () => {
    ...
    it('calls API to fetch projects on mount', async () => {
      ...
      server.use(
        http.get('/api/crm/projects', ({ request }) => {
          apiCallSpy(request.url)
          return HttpResponse.json([])
        })
      )
      ...
      await waitFor(() => {
        expect(apiCallSpy).toHaveBeenCalledWith('http://localhost:3000/api/crm/projects')
      })
    })
  })
  ...
  describe('CandidatesPage', () => {
    ...
    it('calls API to fetch candidates on mount', async () => {
      ...
      server.use(
        http.get('/api/crm/candidates', ({ request }) => {
          apiCallSpy(request.url)
          return HttpResponse.json([])
        })
      )
      ...
      await waitFor(() => {
        expect(apiCallSpy).toHaveBeenCalledWith('http://localhost:3000/api/crm/candidates')
      })
    })
  })
})